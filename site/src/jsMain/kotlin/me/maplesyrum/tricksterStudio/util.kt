...
class Attribute<T>(val name: String, val intoFn: (String) -> T) {
    operator fun getValue(thisRef: HTMLElement, property: KProperty<*>): T? {
        return thisRef.getAttribute(name)?.let(intoFn)
    }

    operator fun setValue(thisRef: HTMLElement, property: KProperty<*>, value: T?) {
        return thisRef.setAttribute(name, value.toString())
    }
}

class DefaultedAttribute<T>(val name: String, val intoFn: (String) -> T, val default: T) {
    operator fun getValue(thisRef: HTMLElement, property: KProperty<*>): T {
        return thisRef.getAttribute(name)?.let(intoFn) ?: default
    }
